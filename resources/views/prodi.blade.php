            ...
            <div class="relative flex flex-col min-w-0 break-words bg-white border-0 shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
                <div class="border-black/12.5 rounded-t-2xl border-b-0 border-solid p-6 pb-0">
                    <div class="flex justify-between items-center mb-4">
                        <h2 class="text-2xl font-bold dark:text-white">Data Daftar Jurusan</h2>
                        <a href="{{ url('prodiadd') }}" class="inline-block px-6 py-2 font-bold text-white bg-blue-500 rounded-lg hover:bg-blue-700">Tambah</a>
                    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        $(document).ready(function() {
                ...
                .then(function(response) {
                    if (response.data && Array.isArray(response.data.data)) {
                        response.data.data.forEach(function(item) {
                            ...
                            var kode_jurusan = escapeHtml(item.kode_jurusan);
                            var jurusan = escapeHtml(item.jurusan);
                            table.row.add([
                                kode_jurusan,
                                jurusan,
            ...
            $('#kurikulumTable').on('click', '.details-btn', function() {
                ...
                // Redirect hanya jika jurusanId ada
                if (jurusanId) {
                    window.location.href = '/detailsprodi/' + jurusanId;
                } else {
            ...
            $('#kurikulumTable').on('click', '.delete-btn', function() {
                ...
                var jurusanId = deleteButton.data('id');
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // Panggil API delete
                        axiosInstance.delete(`/jurusan/${jurusanId}`)
                            .then(function(response) {
